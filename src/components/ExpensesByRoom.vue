...
const getExpensesByRoomId = async (pageNo:number) => {
    await expensesService.getExpensesByRoomId(props.year, props.roomId,pageNo).then(response => {
        expensesDetail.value = response.content.map((expenses: { month: any; rentalFee: any; electricPreviousMeter: any; electricCurrentMeter: any; waterPreviousMeter: any; waterCurrentMeter: any; debt: any; fine: any; status: any; internet: any; service: any; securityDeposite: any; }) => ({
